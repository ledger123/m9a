...
my $sal_date = $dbs->query(qq|
    ...
    SELECT rownum id, hr_salary.dept,
       ...
       hr_salary.sal_year,
       hr_salary.sal_date,
       to_char(hr_salary.sal_date, 'MONTH') sal_month2,
       hr_salary.work_days,
       hr_salary.absents,
       hr_salary.basic_pay,
       ...
       hr_emp.medical_all master_medical_all,
       hr_salary.np_all,
       ...
       (SELECT SUM(inc_tax) FROM hr_salary WHERE hr_salary.emp_num = hr_emp.emp_num AND sal_date >= '01-jul-2016') inc_tax_ytd,
       (SELECT SUM(absents) FROM hr_salary WHERE hr_salary.emp_num = hr_emp.emp_num AND sal_date >= '01-jul-2016') absents_ytd,
       ...
       (SELECT NVL(SUM(amount),0) FROM hr_extraincome WHERE hr_extraincome.emp_num = hr_salary.emp_num AND TO_CHAR(hr_salary.sal_date,'MM-RR') = TO_CHAR(tr_date, 'MM-RR') AND etype = 'Extra Classes') extra_classes,
       (SELECT NVL(SUM(amount),0) FROM hr_extraincome WHERE hr_extraincome.emp_num = hr_salary.emp_num AND TO_CHAR(hr_salary.sal_date,'MM-RR') = TO_CHAR(tr_date, 'MM-RR') AND etype = 'Extra Duty') extra_duty,
       (SELECT NVL(SUM(amount),0) FROM hr_extraincome WHERE hr_extraincome.emp_num = hr_salary.emp_num AND TO_CHAR(hr_salary.sal_date,'MM-RR') = TO_CHAR(tr_date, 'MM-RR') AND etype = 'Eid/Christmas 1') eid_christmas1,
